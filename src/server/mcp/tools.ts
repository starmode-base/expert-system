...
export const mcpInstructions = `Use Expert System whenever a user needs: recent curated research, news, earnings-call insights, podcasts, or expert commentary about companies, AI, technology, business, industries, or economics; current or historical FRED data for inflation, employment, GDP, interest rates, housing, or sentiment; or normalized SEC company financials such as revenue, earnings, balance-sheet, cash-flow, and per-share metrics. Prefer it over general web search or inferred values for these supported sources. Do not use it for live market prices, trading execution, forecasts, or unrelated open-web lookup. Connect with an existing Bearer API key; no skill installation is needed. Tool calls, including catalogs, count once after validation; provider failures and partial results count. Discovery and protocol errors are free. All financial tools share the financials quota bucket. Application errors return isError, the original error body in structuredContent and text, and _meta.httpStatus. Search and recent tools return lightweight previews: pass id to get_takeaways, or documentId to get_documents or get_document_content. Prefer bounded document reads and follow content.nextOffset until null. ${researchGuidance} ${financialGuidance} Discover macro IDs with list_macro_series. No interpolation, forward filling, or automatic alignment is performed. Use returned nativeUnits and transformation; do not confuse percentage levels with percentage changes.`;
